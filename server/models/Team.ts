...
export enum TeamQueries {
  ...
  GET_BY_ID = "SELECT team.id, team.name, team.clubId, (SELECT JSON_OBJECT('name', c.name, 'stadium', c.stadium, 'logoUrl', c.logoUrl) FROM club c WHERE c.id = team.clubId) AS club FROM team WHERE id = ?",
}
